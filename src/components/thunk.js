  ...
  const style={ backgroundColor: 'black', color: 'white' }
    return (
      <div>
        ...
        <p>This last point is really important: you should always call <code class="language-text">next(action)</code> in your middleware. If you forget it, Redux stops, and no other action will reach the reducer. <code class="language-text">next(action)</code> moves the application forward by <strong>calling the next middleware in the chain</strong>.</p>
        <div data-language="javascript" style={style}>
          <pre>
            <code class="language-javascript">
              <span>function thunkMiddleware</span>
        ...
        <p>What's also interesting is that the middleware doesn't exit after <code class="language-text">next(action)</code>. If you're interested in <strong>reading the next state of the app after the middleware chain runs</strong> you can capture it with <code class="language-text">getState</code> after <code class="language-text">next(action)</code>:</p>
        <div data-language="javascript" style={style}>
          <pre>
            <code class="language-javascript">
              <span>function thunkMiddleware</span>
        ...
        <p>Here’s what the middleware does: when action type is <code class="language-text">ADD_ARTICLE</code> check if <code class="language-text">action.payload.title</code> contains a bad word. If it does then dispatch an action of type <code class="language-text">FOUND_BAD_WORD</code>, otherwise <strong>let the next middleware run</strong>.</p>
        <div data-language="javascript" style={style}>
          <pre>
            <code class="language-javascript">
        ...
        <p>Now, time to wire up <code class="language-text">forbiddenWordsMiddleware</code> to the Redux store. For that we need to import our middleware, another utility from Redux (applyMiddleware), and then cook everything together.</p>
        <p>Open up <code class="language-text">src/js/store/index.js</code> and modify the file like so:</p>
        <div data-language="javascript" style={style}>
          <pre>
            <code class="language-javascript">
